...
class AsyncWebServerRequest
{
  ...
  public:
    ...
    void send(FS &fs, const String& path, const String& contentType=String(), bool download=false, AwsTemplateProcessor callback=nullptr);//MCB
    void send(File content, const String& path, const String& contentType=String(), bool download=false, AwsTemplateProcessor callback=nullptr);//MCB
...
typedef std::function<void(AsyncWebServerRequest *request, const String& filename, size_t index, uint8_t *data, size_t len, bool final)> ArUploadHandlerFunction;//MCB
...
class AsyncWebServer
{
  ...
  public:
    ...
    void onFileUpload(ArUploadHandlerFunction fn); //handle file uploads /MCB
