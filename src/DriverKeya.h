...
public:
  ...
  DriverKeya(uint8_t _canId=3, uint32_t _baudRate=250000) {
    ...
    errorCode = K_Bus.begin(settings, filter);//Macro definition (in CANManager.h) of K_Bus=ISO_Bus for ESP32 as only one CAN available
   #endif
   #if MICRO_VERSION == 2
    ACAN_T4_Settings settings (_baudRate) ;
    const ACANPrimaryFilter primaryFilters [] = {
      ACANPrimaryFilter (kData, kExtended, 0x07000001) // Filter #0
      };

    errorCode = (canId==2)? ISO_Bus.begin(settings, primaryFilters, 1) : K_Bus.begin(settings, primaryFilters, 1);
    //0: none, 1: V+ISO+K !Keya, 2: V+ISO+Keya !K, 3: V+Keya+K, 4: V+ISO !K!Keya, 5:V+K !Keya!ISO, 6: V+Keya !K!ISO, 7:Keya !V!K!ISO
   #endif
    ...
    (canId==2)? ISO_Bus.tryToSend(msg) : K_Bus.tryToSend(msg);
  ...
  int8_t getCurrent() {
    CANMessage msg;
    if((canId==2)? ISO_Bus.receive(msg) : K_Bus.receive(msg)){
